...
internal sealed class AuctionMailClaimManager : IAuctionMailClaimManager
{
    ...
    private readonly Func<Character, bool> _persistSourceState;
    ...
    internal AuctionMailClaimManager(
        ...
        Func<Character, bool> persistSourceState = null)
    {
        _store = store;
        ...
        _persistSourceState = persistSourceState ??
            (character => SaveManager.Instance.TryCommitEconomy([character]));
    }
        ...
        ulong specifiedItemId)
    {
        ...
        lock (SaveManager.PersistenceSyncRoot)
            return TryClaimLocked(character, mails, mailId, takeMoney, takeItems, takeAllSelected, specifiedItemId);
    }

    private AuctionMailClaimResult TryClaimLocked(Character character, CharacterMails mails,
        long mailId, bool takeMoney, bool takeItems, bool takeAllSelected, ulong specifiedItemId)
    {
        if (mailId <= 0 || mailId > uint.MaxValue)
        ...
        bool takeAllSelected)
    {
        ...
        lock (accountSyncRoot)
        {
            ...
            else
            {
                ...
                if (!_persistSourceState(character))
                    return AuctionMailClaimResult.Failure;
        ...
        ulong specifiedItemId)
    {
        ...
        lock (SaveManager.PersistenceSyncRoot)
        {
            ...
            else
            {
                ...
                // Persist all source settlement, container, and character state before
                // a receipt can make its attachment durable. The shared gate also
                // excludes autosave and real inventory mutation through live apply.
                if (!_persistSourceState(character))
                    return AuctionMailClaimResult.Failure;
