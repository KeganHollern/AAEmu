...
public sealed class AuctionMailClaimStoreTests : IAsyncLifetime
{
    ...
    public async Task Sale_AchievementWriteFailure_RollsBackEveryClaimWrite_AndAllowsRetry()
    {
        ...
        Assert.Equal(0, await ScalarAsync("SELECT COUNT(*) FROM `mails`"));
    ...
    public async Task Buy_PersistsDestinationAndClearsAttachment_WithDurableReplay(bool mergeStack)
    {
        ...
        await ExecuteAsync($"""
            INSERT INTO `items` (`id`,`type`,`template_id`,`slot_type`,`slot`,`count`,`lifespan_mins`,`owner`,`flags`)
            VALUES ({source.Id},'AAEmu.Game.Models.Game.Items.Item',20,{(int)SlotType.Mail},0,5,0,7,0);
            """);
        var store = new MySqlAuctionMailClaimStore();
        var achievements = LoadAchievements(plan.Character);
    ...
    [Fact]
    public void ReceiptCollision_WithDifferentAmount_IsRejected()
    {
        var plan = CreateSalePlan();
        var store = new MySqlAuctionMailClaimStore();
        var achievements = LoadAchievements(plan.Character);
    ...
    private static AuctionBuyClaimPlan CreateBuyPlan(bool mergeStack)
    {
        ...
        var source = new Item(30001, template, 5) { OwnerId = 7, SlotType = SlotType.Mail, Slot = 0 };
        var destination = mergeStack ? new Item(30002, template, 10) : null;
