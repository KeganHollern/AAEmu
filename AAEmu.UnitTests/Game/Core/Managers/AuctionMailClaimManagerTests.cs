...
public sealed class AuctionMailClaimManagerTests
{
    ...
    [Arguments(false)]
    [Arguments(true)]
    public async Task GetAttached_SourceCheckpointFails_DoesNotClaimOrPublish(bool sale)
    {
        var store = new InMemoryAuctionMailClaimStore();
        var manager = CreateManager(store, persistSourceState: _ => false);
        var session = new RecordingSession();
        var (character, mails) = CreateCharacter(manager, session);
        var mail = sale ? AddSaleMail(character, 250) : AddBuyMail(character, 20, 5).Mail;

        var result = mails.GetAttached(mail.Id, sale, !sale, true);
