...
[NotInParallel]
public sealed class QuestTerminationTests
{
    private const uint ItemTemplateId = 50_001;
    private const int RequiredItemCount = 2;
    ...
    [Test]
    [Arguments(RequiredItemCount, 0)]
    [Arguments(RequiredItemCount + 1, 1)]
    public async Task Complete_SupplyItemWithBothFlags_RemovesOnlyCleanupCount(int initialCount, int expectedCount)
    {
        var owner = CreateOwnerWithItem(initialCount);
        var quest = CreateSupplyItemQuest(owner, true);

        quest.Complete();

        await Assert.That(owner.Inventory.GetItemsCount(ItemTemplateId)).IsEqualTo(expectedCount);
    }

    [Test]
    [Arguments(RequiredItemCount, 0)]
    [Arguments(RequiredItemCount + 1, 1)]
    public async Task Drop_SupplyItemWithBothFlags_RemovesOnlyDestroyWhenDropCount(int initialCount, int expectedCount)
    {
        var owner = CreateOwnerWithItem(initialCount);
        var quest = CreateSupplyItemQuest(owner, false);
    ...
    [Arguments(RequiredItemCount, 0)]
    [Arguments(RequiredItemCount + 1, 1)]
    public async Task Complete_ObjItemGatherWithBothFlags_RemovesOnlyCleanupObjectiveCount(int initialCount, int expectedCount)
    {
        var owner = CreateOwnerWithItem(initialCount);
        var quest = CreateItemGatherQuest(owner, true);
    ...
    [Arguments(RequiredItemCount, 0)]
    [Arguments(RequiredItemCount + 1, 1)]
    public async Task Drop_ObjItemGatherWithBothFlags_RemovesOnlyDestroyWhenDropObjectiveCount(int initialCount, int expectedCount)
    {
        var owner = CreateOwnerWithItem(initialCount);
        var quest = CreateItemGatherQuest(owner, false);
    ...
    private CharacterMock CreateOwnerWithItem(int itemCount)
    {
    ...
    private Quest CreateSupplyItemQuest(CharacterMock owner, bool completing)
    {
        var template = new QuestTemplate { Id = 101 };
        var supplyComponent = new QuestComponentTemplate(template)
        {
            Id = 1_011,
            KindId = QuestComponentKind.Supply
        };
        supplyComponent.ActTemplates.Add(new QuestActSupplyItem(supplyComponent)
        {
            ActId = 1_012,
            ItemId = ItemTemplateId,
            Count = RequiredItemCount,
            Cleanup = true,
            DestroyWhenDrop = true
        });
        template.Components.Add(supplyComponent.Id, supplyComponent);
        AddRewardComponent(template);

        return CreateQuest(template, owner, completing ? QuestComponentKind.Reward : QuestComponentKind.Supply);
    }

    private Quest CreateItemGatherQuest(CharacterMock owner, bool completing)
    {
        var template = new QuestTemplate { Id = 102 };
        var progressComponent = new QuestComponentTemplate(template)
        {
            Id = 1_021,
            KindId = QuestComponentKind.Progress
        };
        progressComponent.ActTemplates.Add(new QuestActObjItemGather(progressComponent)
        {
            ActId = 1_022,
            ItemId = ItemTemplateId,
            Count = RequiredItemCount,
        ...
        var quest = CreateQuest(
            template,
            owner,
            completing ? QuestComponentKind.Reward : QuestComponentKind.Progress);
        quest.Objectives[0] = RequiredItemCount;
        return quest;
    }

    private Quest CreateQuest(QuestTemplate template, CharacterMock owner, QuestComponentKind step)
    {
        return new Quest(
            template,
            owner,
    ...
    private static void AddRewardComponent(QuestTemplate template)
    {
